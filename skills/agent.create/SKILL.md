...
**House style for ALL prose you generate in steps 2-4** (role, responsibilities, results, requirements, style, backstory):

- Never use the em dash character (`—`, U+2014). Use a comma, colon, period, or hyphen instead. The generated agent declares "No em dashes. Ever." as a hard rule, so the prose itself must be clean.
- No AI clichés ("Certainly!", "Great question!", "I'd be happy to", "As an AI", etc.).
- Plain text, no flattery.
