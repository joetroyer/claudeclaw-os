...
export function createMissionTask(
  ...
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `INSERT INTO mission_tasks (id, title, prompt, assigned_agent, status, created_by, priority, created_at, source, source_id)
     VALUES (?, ?, ?, ?, 'queued', ?, ?, ?, ?, ?)`,
  ).run(id, title, prompt, assignedAgent, createdBy, priority, now, source, sourceId);
}
