...
/**
 * Slice 9 Wave 0: the runActions caller now passes the source ('webhook'
 * | 'log-tail' | 'sqlite-poll') and source_id (slug for webhooks, watcher
 * name for the others) so the row records exactly which spec fired it.
 * Both default to null so legacy callers (none in-tree, but defensive for
 * tests) keep working.
 */
function queueMission(
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO mission_tasks (id, title, prompt, assigned_agent, status, priority, created_at, created_by, source, source_id)
       VALUES (?, ?, ?, ?, 'queued', 50, ?, ?, ?, ?)`,
    ).run(id, title, prompt, agent, Math.floor(Date.now() / 1000), createdBy, source, sourceId);
    logger.info({ id, agent, title, createdBy, source, sourceId }, 'watcher: queued mission task');
    return id;
  } finally {
    db.close();
  }
